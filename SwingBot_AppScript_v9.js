/**
 * SwingBot Apps Script — v9
 * ─────────────────────────────────────────────────────────────
 * Changes from v8:
 *   + writeScanLog() updated — SCAN_LOG now includes TOS columns:
 *     Last, SwingΔ%, ATR%, RVol, ATR$, %Change
 *   + SCAN_LOG auto-creates with new 13-column header
 *   All v8 trade journal logic (writeTrade, readTrades,
...
function doGet(e) {
  try {
    ...
    return corsOutput({ status: 'ok', message: 'SwingBot Apps Script v9 online' });
...
// ══════════════════════════════════════════════════════════════
// writeTrade — preserved exactly from v8
// ══════════════════════════════════════════════════════════════
  ...
  try {
    ...
    if (data.direction === 'SELL' && tradeId) {
      ...
      for (var i = 1; i < allData.length; i++) {
        if (allData[i][35] === tradeId) {
          var row   = i + 1;
          var entry = parseFloat(allData[i][6])  || 0;
          var shares= parseFloat(allData[i][5])  || 0;
          ...
          sheet.getRange(row, 10).setValue(exit);
          sheet.getRange(row, 11).setValue(data.date || '');
          sheet.getRange(row, 12).setValue(data.exit_reason || '');
          sheet.getRange(row, 18).setValue(Math.round(pnl * 100) / 100);
          sheet.getRange(row, 19).setValue(Math.round(pnlPct * 100) / 100);
          sheet.getRange(row, 20).setValue(Math.round(rMult * 100) / 100);
          sheet.getRange(row, 21).setValue(daysHeld);
          sheet.getRange(row, 23).setValue(data.self_grade || '');
...
// ══════════════════════════════════════════════════════════════
// readTrades — preserved exactly from v8
// ══════════════════════════════════════════════════════════════
function readTrades() {
  try {
    var ss    = SpreadsheetApp.getActiveSpreadsheet();
...
// ══════════════════════════════════════════════════════════════
// writeScanLog — v9: includes TOS columns
// Columns: Timestamp | Ticker | Result | Reason | Score | Grade |
//          ATR% | SwingΔ% | RVol | ATR$ | %Change | Last | SPY RVol
// ══════════════════════════════════════════════════════════════
function writeScanLog(data) {
  try {
    ...
    // Auto-create with updated 13-column header
    if (!sheet) {
      sheet = ss.insertSheet(SCAN_LOG_TAB);
      var headers = [
        'Timestamp', 'Ticker', 'Result', 'Reason', 'Score', 'Grade',
        'ATR%', 'SwingΔ%', 'RVol', 'ATR$', '%Change', 'Last', 'SPY RVol'
      ];
    ...
    var row = [
      ...
      data.rvol        || '',                         // I: RVol (TOS)
      data.atr_dollar  || '',                         // J: ATR$ (TOS)
      data.pct_change  || '',                         // K: %Change (TOS)
      data.last        || '',                         // L: Last price (TOS)
      data.spy_rvol    || '',                         // M: SPY RVol
    ];
    ...
    if (data.result === 'SIGNAL') {
      resultCell.setBackground('#b7e1cd');
    } else if (data.result === 'SKIP') {
      resultCell.setBackground('#f4c7c3');
    }
...
function testConnection() {
  ...
  Logger.log('SwingBot Apps Script v9');
  ...
  Logger.log('All functions: writeTrade ✅ | readTrades ✅ | writeScanLog ✅ (v9 — TOS columns) | buildDashboard ✅');
}

// ══════════════════════════════════════════════════════════════
// buildDashboard — preserved exactly from v8
// ══════════════════════════════════════════════════════════════
